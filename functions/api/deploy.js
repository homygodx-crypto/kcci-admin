export async function onRequestPost(context) {
  ...
  try {
    ...
    // 파일을 개별 업로드 — TextEncoder로 정확한 UTF-8 인코딩
    const formData = new FormData();
    const manifest = {};

    for (const [filename, content] of Object.entries(files)) {
      const encoded = new TextEncoder().encode(content);
      const blob = new Blob([encoded], { type: getContentType(filename) + ';charset=utf-8' });
...
function getContentType(f) {
  ...
  return { html:'text/html', css:'text/css', js:'application/javascript', json:'application/json' }[e] || 'text/plain';
}
...
async function hashStr(s) {
  return hashBuffer(new TextEncoder().encode(s));
}
